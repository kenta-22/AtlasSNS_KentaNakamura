...
<div class="profile-container">
  <div class="profile-wrapper">
    <div class="profile-icon">
      <img class="icon-img" src="{{ asset('storage/images/' . $profile->images) }}">
    </div>
    ...
    <div class="follow-btns">
      ...
      <!-- 認証中のユーザが自分のプロフィール画面を表示させている場合、プロフィール編集ボタンを表示 -->
       <a class="btn btn-secondary btn-custom-profile" href="/users/profile/{{$profile->id}}/update">編集</a>
      @else
        @if(Auth::user()->isFollowing($profile->id))
        <!-- 該当ユーザをフォローしているならば、フォロー解除ボタンを表示 -->
        <a class="btn btn-danger btn-custom-search" href="/users/{{$profile->id}}/unfollow">フォロー解除</a>
        @else
        <!-- フォローしていないならば、フォローするボタンを表示 -->
        <a class="btn btn-primary btn-custom-search" href="/users/{{$profile->id}}/follow">フォローする</a>
        @endif
      @endif
    </div>
  </div>
